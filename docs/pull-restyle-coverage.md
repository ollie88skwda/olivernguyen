...
| Persistence and refresh behavior | Direct browser write and optimistic state | Existing Supabase reads/realtime plus the server write path preserved; failed actions roll back only themselves while settled and pending weekend state remains visible | `loadPicks`, realtime subscription, `/api/pull/pick`, and the failed POST + failed GET browser test |
