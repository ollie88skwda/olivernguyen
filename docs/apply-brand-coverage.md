...
| Images/data graphics | No external images; ratings, odds, calendar, effort and portfolio visuals retained | Token-derived CSS/SVG; dial text uses `--fs-title` and `--fs-label`; explicit flag in `apply.css` header | `.ap-dial`, `.ap-cellbtn`, `.ap-rung`, `.ap-range`, `.ap-curve`, `.ap-gantt`, `.ap-costbar`; no library chart exists |
