...
| Controls and inputs | Editing, sorting, profile fields, filters, duels and sliders | `BRAND.md §4/§8`; UI primitives plus native controls | `Button`, `Select`, `Input`, `Checkbox`; `.ap-cellbtn` and `.ap-rung` are buttons with `--r-control`; `.ap-range` is a native range with `--ctl-h-sm` |
...
| Images/data graphics | No external images; ratings, odds, calendar, effort and portfolio visuals retained | Route-local encodings mapped to existing tokens; no chart primitive matches these data shapes | `.ap-dial`: `--s-24`, `--accent-hi`, `--border`, `--text-faint`, `--fs-title`, `--fs-label`; `.ap-cellbtn`: `color-mix`, `--surface`, `--on-accent`, `--text-faint`, `--r-control`; `.ap-rung`: `--ctl-h`, `--r-control`, `--accent-hi`, `--surface-2`; `.ap-range`: `--ctl-h-sm`, `--accent`; bars/curve/gantt: `--surface-2`, `--accent-hi`, `--warning`, `--success`, `--border`, `--s-*` |
...
| Responsive behavior | Desktop composition and horizontally readable board on phone | `BRAND.md §1/§5` | `@media (max-width: 767px)` in `apply.css`; `.on-table-wrap` owns board scrolling; 375px coverage |
...
**Unmapped aspects:** none. The dial, ink cells, duel rungs, range sliders, compare bars, marginal curve, gantt, and cost/value bars are owned page-local data encodings. Their geometry is data-derived; their colours, type, spacing, radius, motion, and control sizing use the named Sakura tokens above. Their interactions use native range inputs or semantic buttons, including `role="radio"` and `aria-checked` for the duel scale.
