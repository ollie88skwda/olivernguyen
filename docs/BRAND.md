...
2. **Un-restyled legacy pages are frozen.** Files under `src/pages/` keep their own stylesheets
   until a restyle is explicitly scheduled (see `05-v1-spec.md` §2.3). The explicit Sakura-restyle
   exceptions and their current status live in `docs/redesign-research/16-legacy-restyle.md`; do not
   bleed Sakura into the rest.
