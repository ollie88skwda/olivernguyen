...
2. **Remaining legacy pages are frozen.** Files under `src/pages/` keep their own stylesheets
   (see `05-v1-spec.md` §2.3). `/college` and `/emoji` are the explicit sakura-restyle exceptions.
   Do not bleed sakura into other legacy pages.
