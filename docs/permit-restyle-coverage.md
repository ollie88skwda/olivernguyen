...
| Text and type roles | Legacy heading, labels, and body copy used legacy faces and sizes | `Display`, `MonoLabel`, `SectionHead`, `.on-prose`, and token-backed subheads (`BRAND.md §7`) | `permit.js` roles; lane checks the h1, accessible numbered h2 names, visible kickers, ordered steps, and complete route content |
...
| Links and states | Legacy links inherited generic link styling and had small coarse-pointer targets | `--accent-hi`, mode-neutral `--border`, focus ring tokens, state duration/easing, and the coarse `--ctl-h` hit area (`BRAND.md §1/§2/§6`) | `.pm-guide a`, `.pm-shot-link`; lane checks both mode axes' rendered resting underline, all external hrefs, `target`, `rel`, keyboard focus, prose hover underline, media-frame hover border, and 44px coarse targets |
