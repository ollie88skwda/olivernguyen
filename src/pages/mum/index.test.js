...
describe("/mum", () => {
  test("a first visit opens on the animation and taps into reason one", () => {
    render(<MumFifty />);
    expect(screen.getByText("Happy Birthday, Mum")).toBeInTheDocument();
  ...
  test("taps forward and back through the deck, remembering progress", () => {
    ...
    unmount();
    expect(window.localStorage.getItem("mum50.seenOpening")).toBe("1");
    expect(JSON.parse(window.localStorage.getItem("mum50.opened"))).toEqual([1, 2]);
  });
  ...
  test("wiped storage falls back to the first-visit path", () => {
    window.localStorage.setItem("mum50.opened", "not json");
    window.localStorage.setItem("mum50.index", "banana");
    render(<MumFifty />);
    expect(screen.getByText("Happy Birthday, Mum")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: /begin/i }));
    expect(screen.getByText(REASONS[0])).toBeInTheDocument();
